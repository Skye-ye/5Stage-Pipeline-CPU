        addi x29, x0, 0         # x29 = external interrupt counter (initially 0)
        li   t0, %lo(handler)
        csrrw zero, mtvec, t0   # Set interrupt vector to external_handler
        li   t0, 0x8            # MIE bit (bit 3) in mstatus
        csrrs zero, mstatus, t0 # Set mstatus.MIE = 1
        li   t0, 0x800          # MEIE bit (bit 11) for external interrupts
...
handler:csrrs t0, mcause, zero
        ...
        addi  x29, x29, 1
other:  mret
# External interrupt handler test
# Counts the number of external interrupts and stores the count in register x29
# Uses external interrupt cause code 0x8000000b