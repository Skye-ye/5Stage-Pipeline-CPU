            ...
            loop1:  and     x7, x15, x4         # a = sortedstuno & mask0, get the BCD to be processed                                 
                    slli 	x9, x2, 2           # (4 * i)                                                                               
            ...
            loop2:  beq     x3, x11, checkswap  #  to check if j == 8                                                                    
        ...
        incrLoop2:  slli 	x5, x5, 4           #  mask1 = mask1 << 4                                                                   
                    addi    x3, x3, 1           #  j = j + 1                                                                           
                    jal     x0, loop2                                                                                                   
        checkswap:  slt     x14, x2, x12        #  to check if the position of the largest BCD in the this loop has been changed        
                    beq     x14, x0, incrLoop1                                                                                          
                    jal     x1, swap                                                                                                    
        incrLoop1:  slli 	x4, x4, 4           #  mask0 = mask0 << 4                                                                    
                    addi    x2, x2, 1           #  i = i + 1                                                                             
                    bne     x2, x11, loop1      #  to check if i <> 8                                                                    
            result: sw      x15, 4(x0)        	#  [0x04] = sortedstuno                                                                  
                    addi    x2, x0, 0xff
                    slli    x2, x2, 8
                    addi    x2, x2, 0xff
...
dispsortedstuno:    lw      x3, 4(x0) 			#  load the sorted student no.                                                           
                    jal     x0, displayseg7label                                                                                         
dispstuno:          lw      x3, 0(x0) 			#  load the orginal student no.                                                          
